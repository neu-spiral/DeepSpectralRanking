...
class deep_l2_ADMM(object):
    ...
    def create_base_network(self, basenet="googlenet", no_of_layers=2, init_last_layer_weights=None):
        ...
        if basenet == "googlenet":
            ...
            weight_net = Model(input, comp_out)
        weight_net.compile(loss="mean_squared_error", optimizer=Adam(self.lr))
        weight_net.summary()
        return weight_net
    ...
    def fit_deep(self, rho, weights=None, x_beta_b=None, u=None, gamma=1, max_inner_iter=n_iter):
        ...
        while not params_conv:
            ...
            current_output = weights[self.train_samp] - u[self.train_samp]
            start = time()
            self.weight_net.fit(current_input, current_output, batch_size=self.batch_size, epochs=1)
            end = time()
        ...
        print("\n ********* Deep l2 ADMM no of epochs for inner convergence", iter)
        ## dual update
        start = time()
        ...
        return weights, x_beta_b, u, ilrsx_time, nn_time, u_time
